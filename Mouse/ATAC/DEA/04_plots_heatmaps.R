...
dea_results <- read.table("atac_dea_results.txt", sep = "\t", header = TRUE)
...
log_cpm <- read.table("log_cpm.txt", header=TRUE, row.names=1)
...
library(circlize)
setwd("/ibex/user/kurowsaa/Riney_project/Mouse/ATAC/DEA/")
pdf("atac_dea_heatmap.pdf", width=10, height=8)
