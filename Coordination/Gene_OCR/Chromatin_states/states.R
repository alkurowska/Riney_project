...
overlaps <- findOverlaps(states_gr, MGUS_HC_up, minoverlap=40) # 1207
...
final_states <- c("E1","E3","E4","E2","E6","E5","E7","E9","E8","E10","E12","E11")
library(dplyr)
library(tidyverse)

setwd("/ibex/user/kurowsaa/Riney_project/Coordination/Gene_OCR/Chromatin_states")
...
for(i in 1:length(list_toPlot)){
    toPlot <- as.data.frame(list_toPlot[[i]])
    toPlot$Gene <- rownames(toPlot)
    
    # Reshape for ggalluvial
    genes_long <- tidyr::pivot_longer(
    toPlot,
    cols = colnames(toPlot[,1:(ncol(toPlot)-1)]),
    names_to = "Condition",
    values_to = "Category"
    )

    # Order conditions
    genes_long$Condition <- factor(genes_long$Condition, levels = metadata$sample)
    # Order regions
    genes_long$Category <- factor(genes_long$Category, levels = order.chrom.states)
    genes_long$chrom_states_name <- NA 
    for(k in 1:nrow(genes_long)){
         genes_long$chrom_states_name[k] <- chr.states.names[which(order.chrom.states==genes_long$Category[k])]
    }
    genes_long$chrom_states_name <- factor(genes_long$chrom_states_name, levels = rev(chr.states.names))

    p <- ggplot(genes_long, aes(
    x = Condition, stratum = chrom_states_name, alluvium = Gene,
    fill = chrom_states_name, label = chrom_states_name)) +
    ...
    # change x axis labels 45 degress
    theme_classic() +
    labs(x = NULL, y = "Region Count", fill = "Region") +
    scale_fill_manual(values = color_states) 
    ggsave(p, filename = paste0("sankey_plot_", names(list_toPlot)[i], ".pdf"), width = 8, height = 4)
