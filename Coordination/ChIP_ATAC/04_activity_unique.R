...
toKeep <- unique(c(SMM_HC, MM_HC)) #13992
# remove appearing in MGUS_HC
toKeep <- toKeep[!toKeep %in% MGUS_HC] #9109
toPlot <- toPlot[toKeep,]

data <- unique(c(rownames(toPlot)[which(toPlot$SMM_HC>0)],
                   rownames(toPlot)[which(toPlot$MM_HC>0)]))

length(data) #4500 ATAC peaks
...
data_chip <- unique(peaks_ass[peaks_ass$peak_ID%in%data,]$chip_ID) #4440 ChIP peaks
...
setwd("/ibex/user/kurowsaa/Riney_project/Coordination/ChIP_ATAC/PLOTS")
pdf("Up_peaks_log_cpm_ATAC_unique.pdf")
...
setwd("/ibex/user/kurowsaa/Riney_project/Coordination/ChIP_ATAC/PLOTS")
pdf("Up_peaks_log_cpm_ChIP_unique.pdf")
# Plotting the violin plot
...
pdf("Up_peaks_log_cpm_ATAC_ChIP_unique.pdf", width = 7.5, height = 5)
...
setwd("/ibex/user/kurowsaa/Riney_project/Coordination/ChIP_ATAC/PLOTS")
pdf("Up_peaks_raw_ATAC_unique.pdf")
# Plotting the violin plot
ggplot(raw, aes(x = Stage, y = value, fill = Stage)) +
  ...
  scale_y_break(c(200, 5800))  # This creates the visual axis break
...
setwd("/ibex/user/kurowsaa/Riney_project/Coordination/ChIP_ATAC/PLOTS")
pdf("Up_peaks_raw_ChIP_unique.pdf")
# Plotting the violin plot
ggplot(raw, aes(x = Stage, y = value, fill = Stage)) +
  geom_violin(trim = FALSE) +
  # Add mean points
  ...
  #   breaks = c(0, 50, 100, 150, 200, 250, 50000), # Define visible y-axis ticks
  #   labels = c("0", "50", "100", "150", "200", "250", "50000")) +
  scale_y_break(c(200, 7344))  # This creates the visual axis break
...
pdf("Up_peaks_raw_ATAC_ChIP_unique.pdf", width = 7.5, height = 5)
ggplot(mean_values,  aes(x = Stage)) +
  ylim(0, 60) +
