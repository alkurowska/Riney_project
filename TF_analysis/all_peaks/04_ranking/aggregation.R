...
TF_MGUS_rank <- TF_m[,c("OR_MGUS_HC","abs_NES_MGUS_HC", "chromVAR_logFC_MGUS_HC")]
TF_SMM_rank <- TF_m[,c("OR_SMM_HC","abs_NES_SMM_HC", "chromVAR_logFC_SMM_HC")]
TF_MM_rank <- TF_m[,c("OR_MM_HC","abs_NES_MM_HC", "chromVAR_logFC_MM_HC")]
...
SMM_list <- list(rownames(TF_SMM_rank[order(as.numeric(TF_SMM_rank[,1]), decreasing = T),]),
                ...
                rownames(TF_SMM_rank[order(as.numeric(abs(TF_SMM_rank[,3])), decreasing = T),]))
...
MM_list <- list(rownames(TF_MM_rank[order(as.numeric(TF_MM_rank[,1]), decreasing = T),]),
                ...
                rownames(TF_MM_rank[order(as.numeric(abs(TF_MM_rank[,3])), decreasing = T),]))



# MGUS 
Rank_MGUS <- aggregateRanks(glist = MGUS_list, N = nrow(TF_data))
rownames(Rank_MGUS)[1:5]  # "JDP2"   "JUND.1" "JUNB.1" "FOSL1"  "NFE2" 


# SMM
Rank_SMM <- aggregateRanks(glist = SMM_list, N = nrow(TF_data))
rownames(Rank_SMM)[1:5] # "JUND.1" "JUNB.1" "JDP2"   "FOSL1"  "BACH1" 

# MM
Rank_MM <- aggregateRanks(glist = MM_list, N = nrow(TF_data))
rownames(Rank_MM)[1:5] # "JUND.1" "JUNB.1" "JDP2"   "FOSL1"  "NFE2" 
...
Rank_MGUS_sig[rownames(Rank_MGUS_sig)%in%Crisp,] # JUND.1 FOS POU6F1 POU2F2 IRF4
Rank_SMM_sig[rownames(Rank_SMM_sig)%in%Crisp,] # JUND.1 FOS IRF4
Rank_MM_sig[rownames(Rank_MM_sig)%in%Crisp,] # JUND.1 FOS IRF4 POU2F2 POU2F1 IRF2
