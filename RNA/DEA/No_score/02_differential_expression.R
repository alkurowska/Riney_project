...
length(which(contrast_stats$adj.P.Val < 0.05)) #8559
sum(contrast_stats$adj.P.Val < 0.01) #4918
...
length(rownames(dea_results)[rowSums(abs(sig)!=0)>0])  #8528
list_genes <- rownames(dea_results)[apply(sig,1,FUN=function(x){return(sum(x!=0))})>0]
length(list_genes) #8528
#with a adj.pval<0.05 & |FC|>1.5 (logFC>0.58)

setwd("/ibex/user/kurowsaa/Riney_project/RNA/DEA/No_score")
write.table(dea_results,"rna_dea_results_noscore.txt",sep="\t", dec=".", quote=FALSE, row.names=TRUE, col.names=TRUE)

# How many differential genes per transition
length(which(dea_results$sig_MM_SMM != 0)) # 370
length(which(dea_results$sig_SMM_MGUS != 0)) # 202
length(which(dea_results$sig_MGUS_HC != 0)) # 1373
length(which(dea_results$sig_MM_HC != 0)) # 5068
length(which(dea_results$sig_SMM_HC != 0)) # 3672
length(which(dea_results$sig_MM_MGUS != 0)) # 4478
