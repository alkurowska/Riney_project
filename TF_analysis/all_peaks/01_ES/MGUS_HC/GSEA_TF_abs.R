...
logFC <- dea_results$logFC_MGUS_HC
names(logFC) <- rownames(dea_results)
...
# absolute
ranked_abs <- sort(abs(ranked_logFC), decreasing = T)
res <- foreach(i = 1:length(custom_pathways), .packages = c("fgsea"), .combine = rbind) %dopar% {
    ...
    fgseaRes <- fgsea::fgsea(pathways = custom_pathways[i], scoreType = "pos",
                           stats = ranked_abs, minSize = 10, maxSize = length(ranked_logFC) - 1,
                           nPermSimple = 100000)
}
...
setwd("/ibex/user/kurowsaa/Riney_project/TF_analysis/all_peaks/01_ES/MGUS_HC")
save(res, file = "MGUS_HC_GSEA_TF_abs.RData")

# Stop the cluster
stopCluster(cl)
registerDoSEQ()  # Return to sequential processing

load("MGUS_HC_GSEA_TF_abs.RData")
res <- as.data.frame(res)
res <- res[,c("pathway", "ES", "NES", "pval", "p.adjust")]
write.table(res, "MGUS_HC_GSEA_TF_abs.txt", sep = "\t", col.names = T, row.names = F)
