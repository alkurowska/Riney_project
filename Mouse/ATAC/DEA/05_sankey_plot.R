...
mouse_dea <- read.table("atac_dea_results.txt", sep = "\t", header = TRUE)
dim(mouse_dea) # 51685
...
png("peak_sanky.png", width=2000, height=1000, res=300)

# Plot with spacer strata dropped
ggplot(genes_long[!grepl("spacer", genes_long$Display_Condition), ], aes(
  x = Display_Condition, stratum = Category, alluvium = Peak,
