...
my_counts_matrix <- my_counts_matrix[rownames(y$counts),]
...
setwd("/ibex/user/kurowsaa/Riney_project/TF_analysis/all_peaks/03_chromVAR")
...
summary(rowSums(ocr_jaspar_matrix)==0) 
...
dev.off()
quantile(variability$variability,0.25) # cut-off: 3.093402
...
# Limma analysis
load("20250329_dev_jaspar_dev.RData")
...
# Down           7   51  60  55    0     0     0     0          0           0
# NotSig       467  433 412 429  484   484   484   484        484         484
# Up            10    0  12   0    0     0     0     0          0           0
#        gsva frip
# Down      0   26
# NotSig  484  136
# Up        0  322
...
length(which(contrast_stats$adj.P.Val < 0.05)) #39
sum(contrast_stats$adj.P.Val < 0.01) #18
...
length(which(dea_results$sig_SMM_HC != 0)) # 55
length(which(dea_results$sig_MGUS_HC != 0)) # 51

table(dea_results$sig_SMM_HC)
table(dea_results$sig_MGUS_HC)

length(intersect(which(dea_results$sig_SMM_HC == 1), which(dea_results$sig_MGUS_HC == 1))) # 0
length(intersect(which(dea_results$sig_SMM_HC == -1), which(dea_results$sig_MGUS_HC == -1))) # 39
...
summary(decideTests(fit2))
#       Intercept MGUS  MM SMM male qaber del1p del17 trans_4_14 trans_14_16
# Down           6   73 114  87    0     0     0     0          0           0
# NotSig       430  411 358 397  484   484   484   484        484         484
# Up            48    0  12   0    0     0     0     0          0           0
#        frip
# Down     27
# NotSig  186
# Up      271
...
head(contrast_stats, 20)
length(which(contrast_stats$adj.P.Val < 0.05)) #71
sum(contrast_stats$adj.P.Val < 0.01) #36
...
length(rownames(dea_results)[rowSums(abs(sig)!=0)>0]) #128
list_genes <- rownames(dea_results)[apply(sig,1,FUN=function(x){return(sum(x!=0))})>0]
length(list_genes) #128
#with a adj.pval<0.05 & |FC|>2 (logFC>0.58)
...
# How many differential for each contrast
length(which(dea_results$sig_MM_HC != 0)) # 126
