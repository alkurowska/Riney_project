...
#voom transformation
setwd("/ibex/user/kurowsaa/Riney_project/ChIP/DEA/GSVA")
png("voom.png", width = 800, height = 600)
v <- voom(y, design, plot=TRUE)
dev.off()

norm_data <- v$E
dim(norm_data) # 93674 x 128

# Save the data

save(norm_data, v, consensus_peaks, fish_chip, file = "voom_to_dea.RData")
...
myContrMatrix = makeContrasts(
   ...
   SMM_MGUS = SMM, #trasitiion
   MM_MGUS = MM, #transition
...
contrast_stats <- topTable(fit.contrasts2, number=nrow(chip_data), sort.by="none")

head(contrast_stats, 20)
length(which(contrast_stats$adj.P.Val < 0.05)) #125
sum(contrast_stats$adj.P.Val < 0.01) #5

dtest <- decideTests(fit.contrasts2)

contr.matrix <- myContrMatrix
all_padj <- matrix(ncol=ncol(contr.matrix), nrow=nrow(chip_data))
all_fch <- matrix(ncol=ncol(contr.matrix), nrow=nrow(chip_data))
sig <- matrix(0, ncol=ncol(contr.matrix), nrow=nrow(chip_data))

for(i in 1:ncol(contr.matrix)){
  target_stats <- topTable(fit.contrasts2, coef=i,number=nrow(chip_data), sort.by="none")
...
length(rownames(dea_results)[rowSums(abs(sig)!=0)>0])  #1548
list_genes <- rownames(dea_results)[apply(sig,1,FUN=function(x){return(sum(x!=0))})>0]
length(list_genes) #1548
#with a adj.pval<0.05 & |FC|>1.5 (logFC>0.58)

setwd("/ibex/user/kurowsaa/ChIP/DEA/GSVA")
write.table(dea_results,"chip_dea_results.txt",sep="\t", dec=".", quote=FALSE, row.names=TRUE, col.names=TRUE)

# How many differential genes per transition
length(which(dea_results$sig_MM_SMM != 0)) # 0
length(which(dea_results$sig_SMM_MGUS != 0)) # 16
length(which(dea_results$sig_MM_MGUS != 0)) # 1545
