...
res <- foreach(i = 1:length(custom_pathways), .packages = c("fgsea"), .combine = rbind) %dopar% {
    ...
    fgseaRes <- fgsea::fgsea(pathways = custom_pathways[i], scoreType = "neg",
...
save(res, file = "MGUS_HC_GSEA_TF_neg.RData")

# Stop the cluster
stopCluster(cl)
registerDoSEQ()  # Return to sequential processing

load("MGUS_HC_GSEA_TF_neg.RData")
res <- as.data.frame(res)
res <- res[,c("pathway", "ES", "NES", "pval", "p.adjust")]
write.table(res, "MGUS_HC_GSEA_TF_neg.txt", sep = "\t", col.names = T, row.names = F)
