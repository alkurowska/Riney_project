...
colnames(TF_m)
TF_MGUS_rank <- TF_m[,c("OR_inv_MGUS_HC","abs_NES_MGUS_HC", "chromVAR_logFC_MGUS_HC")]
TF_SMM_rank <- TF_m[,c("OR_inv_SMM_HC","abs_NES_SMM_HC", "chromVAR_logFC_SMM_HC")]
TF_MM_rank <- TF_m[,c("OR_inv_MM_HC","abs_NES_MM_HC", "chromVAR_logFC_MM_HC")]
...
MGUS_list <- list(rownames(TF_MGUS_rank[order(as.numeric(TF_MGUS_rank[,1]), decreasing = T),]),
                ...
                rownames(TF_MGUS_rank[order(as.numeric(abs(TF_MGUS_rank[,3])), decreasing = T),]))
...
SMM_list <- list(rownames(TF_SMM_rank[order(as.numeric(TF_SMM_rank[,1]), decreasing = T),]),
                ...
                rownames(TF_SMM_rank[order(as.numeric(abs(TF_MGUS_rank[,3])), decreasing = T),]))
...
MM_list <- list(rownames(TF_MM_rank[order(as.numeric(TF_MM_rank[,1]), decreasing = T),]),
                ...
                rownames(TF_MM_rank[order(as.numeric(abs(TF_MGUS_rank[,3])), decreasing = T),]))



# MGUS 
Rank_MGUS <- aggregateRanks(glist = MGUS_list, N = nrow(TF_data))
rownames(Rank_MGUS)[1:5]  # "NFKB2"  "ZNF524" "ZBTB7A" "NFKB1"  "ETV6"  

Rank_SMM <- aggregateRanks(glist = SMM_list, N = nrow(TF_data))
rownames(Rank_SMM)[1:5] #  "NFKB2"  "ZNF524" "ZBTB7A" "ELK4"   "NFKB1"

# MM
Rank_MM <- aggregateRanks(glist = MM_list, N = nrow(TF_data))
rownames(Rank_MM)[1:5] #"NFKB2"  "ELK4"   "ZBTB7A" "ZNF524" "NFKB1"
...
Rank_MGUS_sig[rownames(Rank_MGUS_sig)%in%Crisp,] # JUND.1
Rank_SMM_sig[rownames(Rank_SMM_sig)%in%Crisp,] 
Rank_MM_sig[rownames(Rank_MM_sig)%in%Crisp,] 


# Save the data
setwd("/ibex/user/kurowsaa/Riney_project/TF_analysis/04_ranking/down")
