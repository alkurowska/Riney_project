...
setwd("/ibex/user/kurowsaa/Riney_project/RNA/Count_matrix/Model_testing/Anova")

stage <- c("MGUS", "SMM", "MM")

for(i in 1:length(stage)){
  ## Perform PCA only for samples in one stage
  meta_stage <- fish_rna[fish_rna$Stage == stage[i],]
  counts_stage <- log_cpm[,meta_stage$`Sample RNAseq`]
  pca_rna <- prcomp(t(counts_stage))
...
# MGUS
results <- read.table("MGUS_anova_results.txt", header = T, row.names = 1)
# remove X from the begning of column names
colnames(results) <- gsub("X", "", colnames(results))
colnames(results) <- gsub("17p.del", "17p del", colnames(results)) 
colnames(results) <- gsub("1p.del", "1p del", colnames(results))
...
for(l in 1:nrow(results)){
  results_p.adj[l,] <- p.adjust(results[l,], method = "fdr")
  write.table(results_p.adj, "MGUS_anova_results_p.adj.txt")
}
...
p <- ggplot(new_results, aes(y=PC,x=variable,
            label = round(values,4))) +
      geom_tile(fill="#666666") + geom_text(size=2,colour = "black") +
      #scale_fill_gradient2(low = "red", high = "blue",, midpoint = 0.025, name = "p.adj") +
      ylim(rev(levels(new_results$PC))) + 
      scale_x_discrete(expand = c(0, 0)) +
      #scale_y_discrete(expand = c(0, 0)) +
...
ggsave(plot = p, dpi = 1800, filename = "MGUS_anova_p.adj.pdf", width = 125, height = 100, units = "mm")
