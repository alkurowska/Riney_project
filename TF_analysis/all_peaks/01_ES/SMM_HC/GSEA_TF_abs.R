...
logFC <- dea_results$logFC_SMM_HC
...
setwd("/ibex/user/kurowsaa/Riney_project/TF_analysis/all_peaks/01_ES/SMM_HC")
save(res, file = "SMM_HC_GSEA_TF_abs.RData")

# Stop the cluster
stopCluster(cl)
registerDoSEQ()  # Return to sequential processing

load("SMM_HC_GSEA_TF_abs.RData")
res <- as.data.frame(res)
res <- res[,c("pathway", "ES", "NES", "pval", "p.adjust")]
write.table(res, "SMM_HC_GSEA_TF_abs.txt", sep = "\t", col.names = T, row.names = F)
