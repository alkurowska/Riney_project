...
length(which(contrast_stats$adj.P.Val < 0.05)) #35240
sum(contrast_stats$adj.P.Val < 0.01) #20479
...
length(rownames(dea_results)[rowSums(abs(sig)!=0)>0])  #28612
list_genes <- rownames(dea_results)[apply(sig,1,FUN=function(x){return(sum(x!=0))})>0]
length(list_genes) #28612
#with a adj.pval<0.01 & |FC|>1.5 (logFC>0.58)

setwd("/ibex/user/kurowsaa/ATAC/DEA/No_score")
write.table(dea_results,"atac_dea_results_noscore.txt",sep="\t", dec=".", quote=FALSE, row.names=TRUE, col.names=TRUE)
...
length(which(dea_results$sig_MGUS_HC != 0)) # 10094
length(which(dea_results$sig_MM_HC != 0)) # 27323
length(which(dea_results$sig_SMM_HC != 0)) # 19607
length(which(dea_results$sig_MM_MGUS != 0)) # 155
