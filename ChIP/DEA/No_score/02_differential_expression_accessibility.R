...
#voom transformation
setwd("/ibex/user/kurowsaa/Riney_project/ChIP/DEA/No_score")
...
length(which(contrast_stats$adj.P.Val < 0.05)) #5868
sum(contrast_stats$adj.P.Val < 0.01) #834
...
length(rownames(dea_results)[rowSums(abs(sig)!=0)>0])  #10659
list_genes <- rownames(dea_results)[apply(sig,1,FUN=function(x){return(sum(x!=0))})>0]
length(list_genes) #10659
#with a adj.pval<0.05 & |FC|>1.5 (logFC>0.58)

write.table(dea_results,"chip_dea_results_noscore.txt",sep="\t", dec=".", quote=FALSE, row.names=TRUE, col.names=TRUE)

# How many differential genes per transition
length(which(dea_results$sig_MM_SMM != 0)) # 0
length(which(dea_results$sig_SMM_MGUS != 0)) # 1354
length(which(dea_results$sig_MM_MGUS != 0)) # 10447